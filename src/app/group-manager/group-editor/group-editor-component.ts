...
export class GroupEditorComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.route.parent.parent.parent.url.subscribe((urlSegment: any) => {
            if (!_.isEmpty(this.facilities)) {
                this.facility = _.find(this.facilities, (facility) => urlSegment[0].path === facility.value);
    ...
    private autoGenerateGroupNumber() {
        this._communityService.generateGroupNumber(this.facility.value).subscribe((res: any) => {
    ...
    public fetchDepartmentProgramConfig(onInit = false) {
        this.departmentProgramConfigService.getDartmentProgramsConfig().subscribe((response) => {
            ...
            if (this.editType.toLowerCase() === 'create') {
                ...
                this.defaultDepartment = this.getDepartmentByProgram(this.groupProgram.value, this.departmentPrograms);
            }
