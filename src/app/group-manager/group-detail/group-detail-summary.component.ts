...
export class GroupDetailSummaryComponent implements OnInit, OnDestroy {
    ...
    @Input() set _group(group: Group) {
        ...
        this.activeMembers = _.filter(group.cohortMembers, (member) => !member.endDate).map((member) => new Patient(member.patient));
    ...
    public initEditLeaderForm() {
        const staffleader = new FormControl(this.currentLeader.person.uuid, Validators.required);
        const peerleader = new FormControl(this.currentLeader.person.uuid, Validators.required);
        const leaderContacts = new FormControl(this.currentLeader.person.patientPhoneNumber);
        ...
        this.editLeaderForm = new FormGroup({
            leadershipType: new FormControl(this.leadershipType, [Validators.required]),
        ...
        this.editLeaderForm.controls['leadershipType'].valueChanges.subscribe((type) => {
            ...
            if (type === PEER) {
                ...
                if (this.leadershipType === STAFF && this.editLeaderForm.controls['staff'].value === this.currentLeader.person.uuid) {
                    this.editLeaderForm.controls['contacts'].patchValue(this.currentLeader.person.patientPhoneNumber);
                } else {
                    this.editLeaderForm.controls['contacts'].patchValue('');
                }
    ...
    private listenToPeerLeaderControlChanges() {
        this.subscription.add(
            this.editLeaderForm.get('peer').valueChanges.subscribe((newValue) => {
                const member = _.find(this.activeMembers, ($member) => $member.uuid === newValue);
    ...
    public initContactsForm(leader: Patient) {
        const validators = [Validators.pattern(this.r1), Validators.pattern(this.r2), Validators.maxLength(15), Validators.minLength(10)];
    ...
    public get inactiveLeaders() {
        return _.filter(this.group.cohortLeaders, (leader) => leader.endDate !== null);
    }
    ...
    public generateLeaderObject(leader, allMembers = []) {
        ...
        this.leadershipType = this.getLeadershipType(leader.person.uuid, allMembers);
        return currentLeader;
    }
